...
| Phase 41B runner is implementation infrastructure, not a result. | Diagnostic | `src/phase41_inner_validation_candidates.py`, `tests/test_phase41_inner_validation_candidates.py` | The Phase 41B runner supports inner-validation-selected candidate experiments for the global/classical ladder. Smoke outputs are engineering diagnostics only; a full development run is required before comparing candidates. |
